...
export default async ({ key, video, }) => {
    const scriptPath = path.join(video['volume'][key], 'Workarea', 'Video', video.slug, 'Metadata', 'Script.nir');
    // attempt to read the file
    const { err, returned } = await promise.handle(fs.readFile(scriptPath, 'utf-8'));
