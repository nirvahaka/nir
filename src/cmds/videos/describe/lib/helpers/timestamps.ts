...
export default async stamps => {
    ...
    const sortedStamps = Object.keys(stamps).sort((a: string, b: string) => {
    ...
    // follow sorted timestamps and populate our returnable
    for (const time of sortedStamps) returnable.push(`${time}   ${stamps[time]}`)
