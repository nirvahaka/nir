...
const setDatabaseURL = async (url) => {
    ...
    let envStr: string = await utilities.fs.exists(envPath) ? await fs.readFile(envPath, 'utf-8') : ''

    // figure out if there's an existing value set
    const existingLine = envStr.split('\n').filter(Boolean).find(line => line.startsWith('DATABASE_URL='))
