...
export default async () => {
    ...
    logger.error(
        'An active volume has not been set please set one\n   before using this command\n',
    )
    logger.info(
        `Use the following command :point_down: to set an active volume\n\n   ${chalk.whiteBright.bold(
            'nir config set volumes.active',
        )} ${chalk.gray('<volume_name>')}`,
    )
